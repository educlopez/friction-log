...
this skill for developing `{{REPO}}`.
...
```bash
gh issue list --repo {{REPO}} --label friction --state open --search "in:title Friction:"
```
...
2. **Invalid** — not repo friction, a duplicate, or not actionable. Comment why
   and close the issue.
3. **Skip** — a fix is possible but you should not ship it without @{{OWNER}}
   ...
   Tell @{{OWNER}} the next run stays skipped until they reply with: close as
...
If @{{OWNER}} already replied after a skip, follow that reply. Do not re-skip
